...
针对Eclipse构建的工程，UPSDK以`*.jar`的方式导入到工程中。UPSDK CocosCreator版本的下载包( [Android-CocosCreatorSDK下载](<../Download/Download_Guide.html#cocoscreator> "SDK下载页面"))解压后的目录结构如下：

![ec-1-1](<../img/AndroidStudio_03.png> "ec-1-1")
- `Eclipse`
...
### 二、导入UPSDK主包
#### 1.添加UPSDK文件
以3.0.06版本为例，在`upsdk_ads`目录下，有`libs`，`res`以及`assets`三个目录，请分别将目录下的内容复制到您Eclipse主工程的相应的目录下(如果没有libs目录，则在app文件夹下和src同级的目录中创建libs)。
...
![ec-2-1](<../img/AndroidStudio_04.png> "ec-2-1")

> `UPAdsSdk_CocosCreator_3.0.06.jar`仅作为示例参考说明
